...
def ZeroGoZ(classes, opt):
    inp1 = Input(shape=(None,))
    inp2 = Input(shape=(1,))
    emb = Embedding(input_dim=26, output_dim=23, embeddings_initializer='uniform')(inp1)
    goz = Embedding(input_dim=26, output_dim=23, embeddings_initializer='uniform')(inp2)
    feats = GlobalMaxPooling1D()(Features(emb))
    out = Classifier(Concatenate()[feats, goz], classes)
